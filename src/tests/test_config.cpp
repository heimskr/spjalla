...
namespace spjalla::tests {
	void test_config(haunted::tests::testing &unit) {
		...
		config::database::groupmap &reg = cfg.registered;

		config::database::ensure_config_db();
		...
		unit.check(one.get_type(), config::value_type::string_, "group.one.type");
		...
		unit.check({
			...
			{{"foo =bar "s},  {"foo", "bar"}},
			{{" foo=bar "s},  {"foo", "bar"}},
		}, &config::database::parse_kv_pair, "config::parse_kv_pair");
		...
		unit.check({
			...
			{{""s}, ""},
			{{"\"\""s}, ""},
		}, &config::database::parse_string, "config::parse_string");

		unit.check("config::parse_string(\" \\\" bar\")", typeid(std::invalid_argument),
			"Invalid quote placement in string value", &config::database::parse_string, " \" bar"s);

		unit.check("config::parse_string(\"\\\"\")", typeid(std::invalid_argument),
			"Invalid length of string value", &config::database::parse_string, "\""s);

		unit.check({
			{{"42"}, config::value_type::long_},
			{{"42."}, config::value_type::double_},
			{{"\"foo\""}, config::value_type::string_},
			{{""}, config::value_type::string_},
			{{"\""}, config::value_type::invalid},
			{{"."}, config::value_type::double_},
			{{".."}, config::value_type::invalid},
		}, &config::database::get_value_type, "config::get_value_type");
