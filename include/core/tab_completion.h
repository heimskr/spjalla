...
namespace spjalla {
	 ...
	 * For convenience, the index of the argument the cursor is on is provided, as well as the index within the
	 * argument. If either of them doesn't apply, they'll be -1.
	 */
	using completer = std::function<void(const input_line &, std::string &raw, size_t &index, long arg_index, long index_in_arg)>;
}
