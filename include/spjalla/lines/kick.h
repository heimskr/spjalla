...
namespace spjalla::lines {
	struct kick_line: public line {
		...
		const std::string &reason_, long stamp_, bool is_self_ = false):
			line(stamp_), chan(chan_), who(who_), whom(whom_), reason(reason_), is_self(is_self_) {}
		...
		kick_line(const pingpong::kick_event &ev):
			kick_line(ev.chan, ev.who, ev.whom, ev.content, ev.stamp) {}
