...
namespace spjalla::lines {
	struct nick_change_line: public line {
		...
		nick_change_line(std::shared_ptr<pingpong::user> user_, const std::string &old_name_,
			...
			line(stamp_), user(user_), old_name(old_name_), new_name(new_name_) {}

		nick_change_line(const pingpong::nick_event &ev):
			nick_change_line(ev.who, ev.content, ev.who->name, ev.stamp) {}
